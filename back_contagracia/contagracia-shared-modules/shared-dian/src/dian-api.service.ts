...
export interface SyncCompanyData {
  ...
  phone: string;
  email: string;
  type_document_identification_code: string; // code de la paramétrica (ej: "13")
  type_organization_code: string;            // code (ej: "1")
  type_regime_code: string;                  // code (ej: "48")
  type_liability_code: string;               // code (ej: "O-13")
  municipality_code: string;                 // code DANE
  department_code: string;                   // code DANE
}
...
export class DianApiService {
  private readonly logger = new Logger(DianApiService.name);
   ...
   */
  private filterLargeFields(data: Record<string, any>): Record<string, any> {
    const filtered: Record<string, any> = {};
    for (const [key, value] of Object.entries(data)) {
      if (DianApiService.EXCLUDED_RESPONSE_FIELDS.includes(key.toLowerCase())) {
  ...
  async saveApiLog(
    ...
    try {
      ...
      await (tenantDb as any).apiLog.create({
        data: {
  ...
  async syncCompany(
    companyId: string,
    data: SyncCompanyData,
    tenantContext: TenantContextService,
  ): Promise<SyncCompanyResponse> {
   ...
   */
  async changeInvoiceEnvironment(
    companyId: string,
    token: string,
    invoiceEnvironment: number,
    payrollEnvironment: number,
  ): Promise<{ success: boolean; message?: string }> {
   ...
   * PUT /api/ubl2.1/config/environment
   */
  async changePayrollEnvironment(
    companyId: string,
    token: string,
    invoiceEnvironment: number,
    payrollEnvironment: number,
  ...
  async getNumberingRange(
    token: string,
    softwareId: string,
  ): Promise<{ success: boolean; message?: string; resolutions?: any[]; data?: any }> {
    const result = await this.makeRequest(
      '/api/ubl2.1/numbering-range',
      'POST',
  ...
  ): Promise<{ success: boolean; message?: string; zipKey?: string; data?: any }> {
    const result = await this.makeRequest(
   ...
   */
  async checkZipStatus(
    token: string,
    zipKey: string,
  ): Promise<{ success: boolean; message?: string; statusCode?: string; statusDescription?: string; data?: any }> {
    const result = await this.makeRequest(
      `/api/ubl2.1/status/zip/${zipKey}`,
      'POST',
      { sendmail: false, sendmailtome: false, is_payroll: false, is_eqdoc: false },
      token,
    );
  ...
  ): Promise<{ success: boolean; message?: string; cune?: string; data?: any }> {
    const result = await this.makeRequest(
      `/api/ubl2.1/payroll/${testSetId}`,
      'POST',
      payrollData,
      token,
    );
   ...
   */
  async sendPayrollAdjustNote(
    token: string,
    testSetId: string,
    noteData: Record<string, any>,
  ): Promise<{ success: boolean; message?: string; cune?: string; data?: any }> {
    const result = await this.makeRequest(
      `/api/ubl2.1/payroll-adjust-note/${testSetId}`,
      'POST',
      noteData,
      token,
    );
