import { JournalEntryItemType, JournalEntryItemRefType, BankAccountType } from '@prisma/client-tenant';
 ...
 */
function validateAccountClasses(items: JournalEntryItemInput[]): void {
  const orderClasses = ['7', '8', '9'];

  const hasOrderAccounts = items.some(item =>
    orderClasses.includes(item.account_code.charAt(0))
  );
  const hasRegularAccounts = items.some(item =>
    !orderClasses.includes(item.account_code.charAt(0))
  );

  if (hasOrderAccounts && hasRegularAccounts) {
    throw new BadRequestException(
      'Las cuentas de clase 7, 8 y 9 (orden/producción) solo pueden tener movimientos entre ellas mismas'
    );
  }
}
 ...
 */
async function shouldSkipAccountClassValidation(
  tx: any,
  typeKey: string,
  referenceId?: string
): Promise<boolean> {
  if (typeKey === 'opening_balance') return true;
  ...
  if (typeKey === 'reversal' && referenceId) {
    ...
    if (originalEntry?.type_key === 'opening_balance') return true;
  }
...
export async function createJournalEntry(
  ...
  if (!params.date && !allowNullDate) {
    throw new BadRequestException('La fecha es requerida para este tipo de asiento');
  }
  ...
  if (!allowNullDate && params.date) {
    await validatePeriodOpen(tx, params.date);
  }
  ...
  if (!params.description || !params.description.trim()) {
    throw new BadRequestException('La descripción del asiento es requerida');
  }

  // 1. Validar montos: deben ser > 0 (manuales no permiten 0, ninguno permite negativos)
  for (const item of params.items) {
    if (item.amount < 0) {
      throw new BadRequestException(`El monto de cada línea debe ser mayor a 0 (cuenta: ${item.account_code})`);
    }
    if (item.amount === 0 && isManual) {
      throw new BadRequestException(`Los asientos manuales no pueden tener líneas con monto 0 (cuenta: ${item.account_code || 'sin cuenta'})`);
    }
  ...
  if (items.length < 2) {
    throw new BadRequestException('El asiento debe tener al menos 2 líneas con monto');
  }
  ...
  if (itemsWithoutDescription.length > 0) {
    throw new BadRequestException('Todas las líneas del asiento deben tener descripción');
  }
  ...
  if (missingAccounts.length > 0) {
    throw new BadRequestException(`Cuentas no encontradas: ${missingAccounts.join(', ')}`);
  }

  // 3. Validar terceros si se proporcionan
  const thirdPartyIds = [...new Set(items.map(i => i.third_party_id).filter((id): id is string => !!id))];
  if (thirdPartyIds.length > 0) {
    ...
    if (missingIds.length > 0) {
      throw new BadRequestException('Uno o más terceros no fueron encontrados');
    }
  }

  // 4, 5, 6. Validar bank_account según cuenta contable
  const bankAccountIds = [...new Set(items.map(i => i.bank_account_id).filter((id): id is string => !!id))];
  let bankAccountsMap: Map<string, { id: string; account_type: BankAccountType }> = new Map();
  ...
  if (bankAccountIds.length > 0) {
    ...
    if (missingBankIds.length > 0) {
      throw new BadRequestException('Una o más cuentas bancarias no fueron encontradas');
    }
  }
    ...
    if (item.bank_account_id) {
      ...
      if (is1110) {
        if (bankAccount?.account_type === 'CASH') {
          throw new BadRequestException(`La cuenta ${accountCode} (Bancos) no puede usar una cuenta bancaria tipo Caja`);
        }
      } else if (is1105) {
        if (bankAccount?.account_type !== 'CASH') {
          throw new BadRequestException(`La cuenta ${accountCode} (Caja) solo puede usar cuentas bancarias tipo Caja`);
        }
      } else {
        throw new BadRequestException(`La cuenta ${accountCode} no puede tener cuenta bancaria asociada. Solo cuentas 1110* y 1105* pueden tenerla`);
      }
    }
  }
  ...
  if (!totalDebits.toDecimalPlaces(4).equals(totalCredits.toDecimalPlaces(4))) {
    throw new BadRequestException(`El asiento no está balanceado. Débitos: ${totalDebits.toFixed(4)}, Créditos: ${totalCredits.toFixed(4)}`);
  }

  // 9. Validar clases de cuenta (7, 8, 9)
  const skipClassValidation = await shouldSkipAccountClassValidation(
    tx,
    params.type_key,
    params.reference_id
  );
  ...
  if (!entryType) {
    throw new BadRequestException(`Tipo de asiento '${params.type_key}' no válido`);
  }
  ...
  const entry = await tx.journalEntry.create({
    data: {
      ...
      items: {
        create: items.map((item: JournalEntryItemInput) => ({
          ...
          cost_center_movement_type_key: item.cost_center_movement_type_key || null,
        })),
      },
    },
  });
