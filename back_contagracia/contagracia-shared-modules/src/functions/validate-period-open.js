...
exports.validatePeriodOpen = validatePeriodOpen;
 ...
 */
async function validatePeriodOpen(tenantDb, date) {
    ...
    if (!annualPeriod) {
        ...
        throw new common_1.BadRequestException(`No existe un período contable anual para el año ${year}. Debe crear el período antes de registrar operaciones.`);
    }
    // Si el período anual está cerrado
    if (annualPeriod.status === 'CLOSED') {
        throw new common_1.BadRequestException(`El período contable "${annualPeriod.name}" está cerrado. No se pueden registrar operaciones en este período.`);
    }
    ...
    if (monthlyPeriod && monthlyPeriod.status === 'CLOSED') {
        throw new common_1.BadRequestException(`El período contable "${monthlyPeriod.name}" está cerrado. No se pueden registrar operaciones en este período.`);
    }
 ...
 */
async function isPeriodOpen(tenantDb, date) {
    const operationDate = new Date(date);
    operationDate.setUTCHours(0, 0, 0, 0);
    const annualPeriod = await tenantDb.accountingPeriod.findFirst({
        where: {
