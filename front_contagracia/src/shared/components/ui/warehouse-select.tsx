...
export const WarehouseSelect = React.forwardRef<HTMLDivElement, WarehouseSelectProps>(
  (
    {
      value,
      valueLabel,
    ...
    const [options, setOptions] = React.useState<WarehouseOption[]>([]);
    ...
    const fetchOptions = React.useCallback(async (search: string) => {
      setLoading(true);
      try {
        const data = await warehousesService.getWarehousesForSelect(search || undefined);
        setOptions(data);
      } catch {
        setOptions([]);
      } finally {
    ...
    const handleSelect = (opt: WarehouseOption) => {
    ...
    const handleCreate = async () => {
      ...
      setCreating(true);
      try {
        const created = await warehousesService.create({ name: trimmed });
        toast.success('Almacen creado');
        handleSelect({ id: created.id, name: created.name });
        fetchOptions('');
      } catch (err: any) {
        toast.error(err?.response?.data?.message || 'Error creando almacen');
      } finally {
        setCreating(false);
      }
    };
      ...
      >
        ...
        <div className="max-h-60 overflow-y-auto p-1">
          {loading ? (
            ...
            <div className="py-6 text-center text-gray-500 text-sm">No se encontraron almacenes</div>
          ) : (
            options.map((opt) => (
              <button
                key={opt.id}
