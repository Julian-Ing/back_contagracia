...
import {
  Search, Plus, ArrowDownCircle, ArrowUpCircle, CheckCircle, XCircle, Clock, Eye, ChevronLeft, ChevronRight,
} from 'lucide-react';
import toast from 'react-hot-toast';
import { SearchableSelect } from '@/shared/components/ui/searchable-select';
import { productTransfersService } from '../services/product-transfers.service';
import { CreateProductTransferDialog } from './CreateProductTransferDialog';
import type { ProductTransferItem } from '../types';
...
export const ProductTransfersTab = ({ productId, parentProductId, canCreate, canApproveReject }: ProductTransfersTabProps) => {
  const [transfers, setTransfers] = useState<ProductTransferItem[]>([]);
  ...
  // Detail dialog
  const [selectedTransfer, setSelectedTransfer] = useState<ProductTransferItem | null>(null);
  ...
  const fetchTransfers = useCallback(async () => {
    setLoading(true);
    try {
      const res = await productTransfersService.getAll({
  ...
  }, [productId, search, page, statusFilter]);

  useEffect(() => { fetchTransfers(); }, [fetchTransfers]);

  const handleApprove = async (id: string) => {
    try {
      await productTransfersService.approve(id);
  ...
  const handleReject = async () => {
    ...
    try {
      await productTransfersService.reject(rejectingId, rejectionReason || undefined);
  ...
  const openDetail = (transfer: ProductTransferItem) => {
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input
            placeholder="Buscar por consecutivo, razón, producto..."
            value={search}
            ...
            {loading ? (
              ...
              return (
                <TableRow key={t.id} className="cursor-pointer hover:bg-muted/50" onClick={() => openDetail(t)}>
                  ...
                  <TableCell className="text-sm">{t.requested_by?.name || '-'}</TableCell>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  {selectedTransfer.requested_by?.name || '-'}
                </div>
              ...
              {/* Acciones */}
              {canApproveReject && selectedTransfer.status === 'PENDING' && (
