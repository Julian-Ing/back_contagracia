...
import { InvoiceTestSetModal } from './InvoiceTestSetModal';
...
export const InvoiceSoftwareCard = () => {
  ...
  const canManageSoftware = can('electronic_documents.invoice_software.manage');
  const canManageEnvironment = can('electronic_documents.invoice_environment.manage');
  ...
  const loadConfig = async () => {
    try {
      const response = await electronicDocsClient.get('/software/invoice');
  ...
  const handleSaveSoftware = async () => {
    ...
    setLoading(true);
    try {
      await electronicDocsClient.patch('/software/invoice', {
        software_id: config.software_id,
        software_pin: Number(config.software_pin),
      });

      toast.success('Software de facturación guardado');
      loadConfig();
    } catch (error) {
  ...
  const handleEnvironmentChange = async (env: 1 | 2) => {
    ...
    try {
      const endpoint = env === 1
        ? '/environment/invoice/production'
        : '/environment/invoice/habilitation';

      await electronicDocsClient.patch(endpoint);

      setConfig((prev) => ({ ...prev, environment: env }));
      toast.success(`Facturación en ${env === 1 ? 'PRODUCCIÓN' : 'HABILITACIÓN'}`);
    } catch (error) {
  ...
  const handleImportResolutions = async () => {
    setImportLoading(true);
    try {
      const res = await electronicDocsClient.post('/test-set/invoice/import-resolutions');
      const msg = res.data.message || 'Resoluciones importadas';
      ...
      toast.error(error?.response?.data?.message || 'Error importando resoluciones');
    } finally {
        ...
        {canManageEnvironment && (
          <div className="space-y-2 pt-2 border-t">
            ...
            <div className="flex gap-2">
              ...
              <Button
                onClick={handleImportResolutions}
                disabled={importLoading}
                size="sm"
                variant="outline"
                className="flex-1 h-8"
              >
                {importLoading && <Loader2 className="mr-1 h-3 w-3 animate-spin" />}
                Imp. Resoluciones
              </Button>
              <Button
                onClick={() => {
